...
from sql.process_sql import get_tables_with_alias, parse_sql
...
def read_dataset_values_from_json(db_id: str, dataset_path: str, tables: List[str]):
    ...
    for item in data:
        if item['db_id'] == db_id:
            for table in tables:
            # for table_name, val in item['tables'].items():
